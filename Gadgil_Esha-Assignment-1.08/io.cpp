...
void choose_pokemon() {
  ...
  class Pokemon *choose[3];
  choose[0]=new class Pokemon(1);
  choose[1]=new class Pokemon(1);
  choose[2]=new class Pokemon(1);

 
  mvprintw(0, 0, "Choose your first Pokemon: ");
  mvprintw(1, 3, "1: %s", choose[0]->get_species());
  mvprintw(2, 3, "2: %s", choose[1]->get_species());
  mvprintw(3, 3, "3: %s", choose[2]->get_species());
  ...
  for (i = 0; i < 6; i++)
  {
    world.pc.char_pokemon[i] = NULL;
  }

  switch (input) {
    case '1':
      world.pc.char_pokemon[0] = choose[0];
     
      delete choose[1];
      delete choose[2];
      break;
    case '2':
      world.pc.char_pokemon[0] = choose[1];
     
      delete choose[0];
      delete choose[1];
      break;
    case '3':
      world.pc.char_pokemon[0] = choose[2];
  ...
  mvprintw(0, 0, "You have choosen %s", world.pc.char_pokemon[0]->get_species());
  mvprintw(1, 0, "Level: %d", world.pc.char_pokemon[0]->get_level());
  mvprintw(2, 0, "Stats:");
  mvprintw(3, 3, "HP: %d", world.pc.char_pokemon[0]->get_hp());
  mvprintw(4, 3, "Attack: %d", world.pc.char_pokemon[0]->get_attack());
  mvprintw(5, 3, "Defense: %d", world.pc.char_pokemon[0]->get_defense());
  mvprintw(6, 3, "Special Attack: %d", world.pc.char_pokemon[0]->get_special_attack());
  mvprintw(7, 3, "Special Defense: %d", world.pc.char_pokemon[0]->get_special_defense());
  mvprintw(8, 3, "Speed: %d", world.pc.char_pokemon[0]->get_speed());
  mvprintw(9, 0, "Moves:");
  mvprintw(10, 3, "Move 1: %s", world.pc.char_pokemon[0]->get_move(0));
  mvprintw(11, 3, "Move 2: %s", world.pc.char_pokemon[0]->get_move(1));
...
int minimum_level = 1, maximum_level;

int manhattan_distance = abs(world.cur_idx[dim_x] - 200) + abs(world.cur_idx[dim_y] - 200);

if (manhattan_distance <= 200) {
    maximum_level = (manhattan_distance == 2) ? 1 : manhattan_distance / 2;
} else {
    maximum_level = 100;
    minimum_level = (manhattan_distance == 201) ? 1 : (manhattan_distance - 200) / 2;
}

  wild_pokemon = new Pokemon(rand() % (maximum_level - minimum_level) + minimum_level);

  // Print the stats of the Pokemon
  clear();

  mvprintw(0, 0, "You came across %s", wild_pokemon->get_species());
  mvprintw(1, 0, "Level: %d", wild_pokemon->get_level());
  mvprintw(2, 0, "Stats:");
  mvprintw(3, 3, "HP: %d", wild_pokemon->get_hp());
  mvprintw(4, 3, "Attack: %d", wild_pokemon->get_attack());
  mvprintw(5, 3, "Defense: %d", wild_pokemon->get_defense());
  mvprintw(6, 3, "Special Attack: %d", wild_pokemon->get_special_attack());
  mvprintw(7, 3, "Special Defense: %d", wild_pokemon->get_special_defense());
  mvprintw(8, 3, "Speed: %d", wild_pokemon->get_speed());
  mvprintw(9, 0, "Moves:");
  mvprintw(10, 3, "Move 1: %s", wild_pokemon->get_move(0));
  mvprintw(11, 3, "Move 2: %s", wild_pokemon->get_move(1));
  mvprintw(13, 28, "Press any key to continue");
...
void pokemon_encountered(npc *n) {
  ...
  for(i = 0; i < n->buddies; i++) {
    // Print the stats of the Pokemon
    clear();

    mvprintw(0, 0, "This trainer has %d Pokemon!", n->buddies);
    mvprintw(1, 0, "Pokemon #%d: %s", (i + 1), n->char_pokemon[i]->get_species());
    mvprintw(2, 0, "Level: %d", n->char_pokemon[i]->get_level());
    mvprintw(3, 0, "Stats:");
    mvprintw(4, 3, "HP: %d", n->char_pokemon[i]->get_hp());
    mvprintw(5, 3, "Attack: %d", n->char_pokemon[i]->get_attack());
    mvprintw(6, 3, "Defense: %d", n->char_pokemon[i]->get_defense());
    mvprintw(7, 3, "Special Attack: %d", n->char_pokemon[i]->get_special_attack());
    mvprintw(8, 3, "Special Defense: %d", n->char_pokemon[i]->get_special_defense());
    mvprintw(9, 3, "Speed: %d", n->char_pokemon[i]->get_speed());
    mvprintw(10, 0, "Moves:");
    mvprintw(11, 3, "Move 1: %s", n->char_pokemon[i]->get_move(0));
    mvprintw(12, 3, "Move 2: %s", n->char_pokemon[i]->get_move(1));
