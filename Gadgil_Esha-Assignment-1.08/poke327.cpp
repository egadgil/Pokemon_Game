...
void new_hiker()
{
  ...
  while((c->buddies < 6) && ((rand() % 10) < 6));

  int i;
  for(i = 0; i < c->buddies; i++){
    int minimum_level, maximum_level;
    int manhattan_distance = (abs(world.cur_idx[dim_x] - 200) + abs(world.cur_idx[dim_y] - 200));

    if(manhattan_distance == 2) {
      minimum_level = maximum_level = 1;
    }
    else if(manhattan_distance <= 200) {
      minimum_level = 1;
      maximum_level = manhattan_distance / 2;
    }
    else if(manhattan_distance == 201) {
      minimum_level = 1;
      maximum_level = 100;
    }
    else {
      minimum_level = (manhattan_distance - 200) / 2;
      maximum_level = 100;
    }

    c->char_pokemon[i] = new Pokemon(rand() % (maximum_level - minimum_level) + minimum_level);
  }
...
void game_loop()
{
  ...
  while (!world.quit) {
    ...
    if(p && (world.cur_map->map[d[dim_y]][d[dim_x]] == ter_grass) && (rand() % 100 < 10)) {
      pokemon_found();
    }
