  ...
  while (!world.quit) {
   c = (character *) heap_remove_min(&world.cur_map->turn);
    ...
    if (p && (d[dim_x] == 0 || d[dim_x] == MAP_X - 1 ||
      ...
      world.cur_idx[dim_x] += (d[dim_x] == 0) ? -1 : (d[dim_x] == MAP_X - 1) ? 1 : 0;
      world.cur_idx[dim_y] += (d[dim_y] == 0 && d[dim_x] != 0 && d[dim_x] != MAP_X - 1) ? -1 : 
                            (d[dim_y] != 0 && d[dim_x] != 0 && d[dim_x] != MAP_X - 1) ? 1 : 0;
...
int main(int argc, char *argv[])
{
 /* struct timeval tv;
  ...
  std::string filename;
    ...
    // Pass the filename to the db_parse function
    db_parse(true, filename.c_str());
  return 0;
}
