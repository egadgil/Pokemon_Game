...
void db_parse(bool print, const char *filename) 
{
  ...
  prefix = (char *) realloc(prefix, prefix_len + 1);
   ...
   fgets(line, sizeof(line), f);
    ...
    // Example usage of i2s in parsing and printing for pokemon.csv
for (i = 1; i < 1093; i++) {
    fgets(line, 800, f);
...
if (print && strcmp(filename, "pokemon.csv") == 0) {
     for (i = 1; i < 1093; i++) {
          std::cout << i2s(pokemon[i].id) << ","
              ...
              << i2s(pokemon[i].order) << ","
              << i2s(pokemon[i].is_default) << std::endl;
    }}
     fclose(f);   
...
prefix = (char *) realloc(prefix,
  ...
  if (print && strcmp(filename, "experience.csv")==0 ) {
    f = fopen("experience.csv", "w");
    for (i = 1; i < 601; i++) {
     std::cout << i2s(experience[i].growth_rate_id) << ","
...
prefix = (char *) realloc(prefix, prefix_len + strlen("type_names.csv") + 1);
  ...
  if (print && strcmp(filename, "pokemon_stats.csv")==0 ) {
    ...
    for (i = 1; i < 6553; i++) {
     std::cout << i2s(pokemon_stats[i].pokemon_id) << ","
              << i2s(pokemon_stats[i].stat_id) << ","
              << i2s(pokemon_stats[i].base_stat) << ","
              << i2s(pokemon_stats[i].effort) << std::endl;
    }
    fclose(f);
  }
prefix = (char *) realloc(prefix, prefix_len + strlen("stats.csv") + 1);
  ...
  fclose(f);
   ...
   if (print && strcmp(filename, "stats.csv")==0 ) {
    f = fopen("stats.csv", "w");
    for (i = 1; i < 9; i++) {
       std::cout << i2s(stats[i].id) << ","
              << i2s(stats[i].damage_class_id) << ","
              << stats[i].identifier << ","
              << i2s(stats[i].is_battle_only) << ","
              << i2s(stats[i].game_index) << std::endl;
    }
    fclose(f);
  }
 ...
 if (print && strcmp(filename, "pokemon_types.csv")==0 ) {
    ...
    for (i = 1; i < 1676; i++) {
       std::cout << i2s(pokemon_types[i].pokemon_id) << ","
